...
def get_dataset(name: str, data_root: str='data', return_transform=False, split=['A', 'B', 'C', 'D']):
    ...
    if name=='mnist':
        ...
        from datafree.datasets.imagenet import ImagenetDataset
        num_classes=1000
        train_transform = T.Compose([
        ...
        split_files = os.path.join(data_root, 'imagenet_split')
        ...
        train_dst = ImagenetDataset(val_root, transform=train_transform, split_root=split_files)
        val_dst = ImagenetDataset(val_root, transform=val_transform, split_root=split_files)
    ...
    # if using official downloaded imagenet-32
        ...
        train_dst = datafree.datasets.TinyImageNet(data_root, split='train', transform=train_transform, download=False)
        val_dst = datafree.datasets.TinyImageNet(data_root, split='val', transform=val_transform, download=False)
