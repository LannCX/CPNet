...
parser.add_argument('--sche', default="cos", type=str, help='cos or multistep')
parser.add_argument('--lr_decay_milestones', default="80,120,160,200,240", type=str,
...
def main():
    ...
    if args.gpu is not None:
        os.environ['CUDA_VISIBLE_DEVICES'] = str(','.join([str(x) for x in args.gpu]))
        if len(args.gpu)==1:
            args.gpu = args.gpu[0]
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    name_project = 'datafree-%s/log-%s-%s-%s%s.txt' % (args.method, args.dataset, args.teacher, args.student, args.log_tag)
    wandb_mode = "disabled" if args.debug else None
    wandb.init(project="FastDFKD", name=name_project,
               tags="t1", mode=wandb_mode,
    ...
    # Freeze weight
    # if 'wrn' in args.teacher or 'imagenet' in args.teacher:
    #     for p in teacher.fc.parameters():
    #         p.requires_grad = False
    #     setattr(student, 'fc', 
    #             nn.Sequential(
    #                 nn.Linear(student.fc.in_features, teacher.fc.in_features) \
    #                     if student.fc.in_features!=teacher.fc.in_features else nn.Identity(),
    #                 teacher.fc))
    # elif 'resnet' in args.teacher:
    #     for p in teacher.linear.parameters():
    #         p.requires_grad = False
    #     setattr(student, 'linear', 
    #             nn.Sequential(
    #                 nn.Linear(student.linear.in_features, teacher.linear.in_features) \
    #                     if student.linear.in_features!=teacher.linear.in_features else nn.Identity(),
    #                 teacher.linear))
    # elif 'vgg' in args.teacher: #vgg11->resnet18
    #     for p in teacher.classifier.parameters():
    #         p.requires_grad = False
    #     setattr(student, 'linear', 
    #             nn.Sequential(
    #                 nn.Linear(student.linear.in_features, teacher.classifier.in_features) \
    #                     if student.linear.in_features!=teacher.classifier.in_features else nn.Identity(),
    #                 teacher.classifier))
    ...
    with open(le_name, "rb") as label_file:
        ...
        label_emb = label_emb.to(device).float()
    ...
    if args.method == 'nayer':
        if args.dataset == 'cifar10' or args.dataset == 'cifar100':
            ...
            synthesizer = datafree.synthesis.NAYER(teacher, student, generator,
                                                  ...
                                                  save_dir=args.save_dir, device=device, transform=ori_dataset.transform,
    ...
    optimizer = torch.optim.SGD(student.parameters(), args.lr, weight_decay=args.weight_decay,
                                momentum=0.9)
    if args.sche == "cos":
        # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, 100, eta_min=args.eta_min)
...
def train(synthesizer, model, criterion, optimizer, args):
    ...
    for i in range(args.kd_steps):
        ...
        with args.autocast():
            ...
            #     # imagenet model
            #     if 'imagenet' in args.student:
            #         s_feat = student.module.fc[0](s_feat)
            #     elif 'wrn' in args.student:
            #         s_feat = student.module.fc[0](s_feat)
            #     elif 'resnet' in args.student:
            #         s_feat = student.module.linear[0](s_feat)
            # else:
            #     # imagenet model
            #     if 'imagenet' in args.student:
            #         s_feat = student.fc[0](s_feat)
            #     elif 'wrn' in args.student:
            #         s_feat = student.fc[0](s_feat)
            #     elif 'resnet' in args.student:
            #         s_feat = student.linear[0](s_feat)
            loss_s = criterion(s_out, t_out.detach()) #+ F.mse_loss(t_feat, s_feat)
