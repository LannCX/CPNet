...
class FastMetaSynthesizer(BaseSynthesis):
    ...
    def synthesize(self, targets=None):
        ...
        if workbook.get_worksheet_by_name('W1') is not None:
            worksheet = workbook.get_worksheet_by_name('W1')
        else:
            worksheet = workbook.add_worksheet('W1')
        ...
        for it in range(self.iterations):
            ...
            print("%s - bn %s - oh %s - adv %s" % (
            it, (loss_bn * self.bn).data, (self.oh * loss_oh).data, (self.adv * loss_adv).data))
            worksheet.write(self.ep, it, loss_oh)
            with torch.no_grad():
        ...
        return {"synthetic": [best_inputs]}, end - start, loss.item(), loss_oh.item()
